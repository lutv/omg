<div class="content-wrapper">
  ...
  <section class="content">
        <div class="row">
        <div class="col-lg-12">
<form role="form" action="<?= base_url()?>jurnal/simpan_update_kas" method="post" enctype="multipart/form-data" id="memori">
          <div class="box box-primary">
        <table class="table table-bordered">
  <tr>
    <td width="120px" >No. Jurnal :</td>
    <td><input type="text" id="jurnal" name="jurnal" value="<?php echo $jurnal['tbjurnalmemori_no']?>" autofocus required class="form-control"><a href="#" id="pilihan_sub" class="input-group-addon btn btn-xs btn-flat">pilih</a></td>
  </tr>
  <tr>
    <td>Referensi :</td>
  ...
  <tr>
    ...
    <td><input type="text" name="ket" value="<?= $jurnal['tbjurnalmemori_ket']?>" size="5" class="form-control"></td>
  </tr>
</table>
...
<table class="table table-bordered table-hover">
  ...
  <tbody id="tabel">
    <?php 
    $no=0;
    foreach ($isi_jual as $key) {
     ?>
    <tr><td>
      <div id="control_s<?=$no?>"><input type='text' name='sub[]' id="nama" value="" size='8' data-sub='<?=$no?>' class='nama form-control'></div></td>
      <td><div id="control<?=$no?>"><input type='text' name='acc[]' id='kode<?=$no?>' size="10" value="<?=$key['penjualan_akun']?>" data-index='<?=$no?>' class='acc form-control'></div></td>
      <td><div id="control_i<?=$no?>"><input type='text' name='inv[]' size='8' data-inv='<?=$no?>' class='inv form-control'></div></td>
      <td><input type='text' name='notes[]' id="notes<?=$no?>" data-notes='<?=$no?>' value="<?=$key['penjualan_notes']?>" size='8' class='notes form-control'></td>
      <td><input type='text' name='valas[]' id='valas<?=$no?>' data-valas='<?=$no?>'  size='8' class='valas form-control'></td>
      <td><select name='dk[]' id='dk_<?=$no?>' data-dk='<?=$no?>' onchange="setNilai(this.value)" class='dk form-control'>
        <option value="">D/K</option>
        <option value="debet" <?php if($key['penjualan_ket']=="debet") echo 'selected'?> >D</option>
        <option value="kredit" <?php if($key['penjualan_ket']=="kredit") echo 'selected'?> >K</option>
      </select></td>
      <td><input type='text' name='nilai[]' id="nilai<?=$no?>" value="<?=$key['penjualan_nilai']?>" size="10" class='nilai form-control'></td>
  ...
  <tr>
    ...
    <td> <div id="tot_kred"></div><input id="f_kred" type="hidden" name="tot_kredit"><input type="hidden" name="editable" value="<?=$no?>"></td>
  </tr> 
  </tfoot>
</form>
  </div>
