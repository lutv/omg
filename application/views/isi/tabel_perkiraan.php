...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    table=$('#tabel').DataTable({
      ...
      "order":[],
      "ajax":{
        "url":"<?=base_url('tabel/ajax_list_akun')?>",
        "type":"POST"
      },
      "columnDefs":[
      {
  ...
  function buat_akun(){
   ...
   function induk(id_level, id_induk){
   $.ajax({
          url : "<?=site_url('tabel/lihat_induk/')?>"+id_level,
          type: "GET",
          dataType: "JSON",
          ...
          success: function(data)
          {
            ...
            for (var i=0; i<data.length; i++){
              if (data[i].id==id_induk) {
             $("#induk_update").append('<option value="'+data[i].id+'" selected >'+data[i].label+'</option>');
           }else{
...
$('#level2').on('change', function()
  ...
  function tampil_form_gambar(id_soal){
     $('#form_update_gambar')[0].reset();
    $('.form-group').removeClass('has-error');
    $('.help-block').empty();
    $.ajax({
      url: "<?=site_url('home/ajax_edit/')?>"+id_soal,
      type: "GET",
      dataType:"JSON",
      success: function(data){
        $('[name="id_soal"]').val(data.id_soal);
        $('[name="old_image"]').val(data.gambar);
        $('#modal_update_gambar').modal('show');
